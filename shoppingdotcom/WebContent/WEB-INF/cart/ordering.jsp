...
<form action="orderComfirm.or" method="post">
...
<c:forEach var="cart" items="${cart}">    
    <tr>
      <!-- <td>ott</td> -->
      <td class="text-center" width="200px;"><img src="ProductImg?fname=${cart.pimage}.PNG" width="130px"></td>
      <td class="text-left" width="300px;">${cart.cname}</td>
      <td class="text-center">${cart.poption}</td>
      <td class="text-center">${cart.pcount}</td>
      <td class="text-center">${cart.cprice }</td>
      
   	  <c:set var="total" value="${total+(cart.cprice * cart.pcount)}"></c:set>
   	  <c:set var="ocount" value="${ocount+cart.pcount }"></c:set>
   	  <input type="hidden" name="pcode" value="${cart.pcode}">
      <input type="hidden" name="pname" value="${cart.cname}">
      <input type="hidden" name="pcount" value="${cart.pcount}">
      <input type="hidden" name="pprice" value="${cart.cprice}">
      <input type="hidden" name="poption" value="${cart.poption}">
      <input type="hidden" name="pimage" value="${cart.pimage}">
    </tr>
</c:forEach>  
    <tr>
     <th colspan="6" class="bggray" style="text-align:right;"> 합계 : ${total}원</th>  
       <input type="hidden" name="oprice" value="${total }">
...
<hr>
	...
	<table style="margin-top:0"><caption style="color:black;"><h3>배송지 정보</h3></caption>
		<tr><th>수령인 이름</th><td colspan="2"><input type="text" value="${customer.cname }" name="oname" required="required" autocomplete="off"></td></tr>
		<tr><th>수령인 번호</th><td colspan="2"><input type=text name="otel" required="required"
		pattern="\d{3}-\d{3,4}-\d{4}" placeholder="xxx-xxxx-xxxx" value="${customer.ctel }"
		title="전화번호 형식 xxx-xxxx-xxxx" autocomplete="off"></td></tr>
		<tr><th rowspan="3">배송지 주소</th><td colspan="2"><input type="text" id="postcode" required="required"  name="postcode" placeholder="우편번호" autocomplete="off" value="${customer.cpostcode }">
		<input type="button" onclick="execDaumPostcode()" value="우편번호 찾기" style="width:120pt; float:right;"></td></tr>
		<tr><td colspan="2" style="border-top:0;"><input type="text" id="address" required="required"  name="address" placeholder="주소" autocomplete="off" value="${customer.caddr }"></td></tr>	
		<tr><td colspan="2" style="border-top:0;"><input type="text" id="detailAddress" required="required"  name="detailAddress" placeholder="상세주소" autocomplete="off" value="${customer.cdetailaddr }">
		...
		<br>입금액 : <span style="font-weight:bold;">${total}</span>원</td></tr>
