    ...
    <ul class="navbar-nav">
     ...
     <li class="nav-item dropdown">
      ...
      <li class="nav-item dropdown">
        ...
        <c:if test="${empty cid}"><li><a class="nav-link" href="loginForm.do">로그인</a></li>
        <li><a class="nav-link" href="joinForm.do">회원가입</a></li></c:if>
      ...
      <li><a class="nav-link" id="cartlist" href="cartList.cr?cid=${cid}">장바구니</a></li>
...
<h2 class="cart">주문조회</h2><br>
<div class="row justify-content-center">
	<div class="col-sm-8">
    <table class="table table-responsive text-center">
    <tr style="background: lightgray">
      <th scope="col" width=200px>주문번호</th>
      <th scope="col" width=200px>주문 날짜</th>
      <th scope="col" width=200px>수령인</th>
      <th scope="col" width=350px>상품정보</th>
      <th scope="col" width=500px>배송지 정보</th>
      <th scope="col" width=200px>결제금액</th>
      <th scope="col" width=500px>배송메모</th>
      <th scope="col" width=200px>진행상태</th>
    </tr>
    
    <c:if test="${empty list }">
    	<th colspan="8">주문이 없습니다</th>
    </c:if>
    <c:if test="${not empty list }">
    <c:forEach var="orderlist" items="${list}">
    <tr>
      <td>${orderlist.ocode }</td>
      <td>${orderlist.odate }</td>
      <td>${orderlist.oname }</td>
      <td><a href="orderProInfo.or?ocode=${orderlist.ocode }&cid=${orderlist.cid}" style="color:black;">${orderlist.oinfo }</a></td>
      <td>${orderlist.oaddr }</td>
      <td>${orderlist.oprice }</td>
      <td>${orderlist.omemo }</td>
      <c:if test="${orderlist.odel == 'n'}"><td>주문완료</td></c:if>
      <c:if test="${orderlist.odel == 'y'}"><td>취소완료</td></c:if>
    </tr> </c:forEach>
    </c:if>
</table>
	<div align="center">
		<c:if test="${startPage>pagePerBlock}">
			<button onclick="location.href='allOrderList.or?pageNum=${startPage-1}'">[이전]</button>
		</c:if>
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			<c:if test="${i==currentPage}">
				<button onclick="location.href='allOrderList.or?pageNum=${i}'"
					disabled="disabled">[${i}]</button>
			</c:if>
			<c:if test="${i!=currentPage }">
				<button onclick="location.href='allOrderList.or?pageNum=${i}'">[${i}]</button>
			</c:if>
		</c:forEach>
		<c:if test="${endPage < totalPage }">
			<button onclick="location.href='allOrderList.or?pageNum=${endPage+1}'">[다음]</button>
		</c:if>
	</div>
</div>
</div>
